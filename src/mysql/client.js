...
const { init } = require("./log");

module.exports = RDSClient;
module.exports.literals = require('./literals');

function RDSClient(dbOptions, otherOptions) {
  if (!(this instanceof RDSClient)) {
    if (otherOptions && otherOptions.logger) {
      init(otherOptions.logger,otherOptions.loggerLevel||"info");
    }
    return new RDSClient(dbOptions);
  }
  Operator.call(this);
