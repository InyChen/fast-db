...
proto.query = async function (sql, values) {
  // query(sql, values)
  ...
  try {
    ...
    if (rows.length !== undefined) {
      info('result:', rows.length ,"rows");
      return rows.map(row => {
        const b = {};
        Object.keys(row).forEach(key => {
          b[changeCase.camelCase(key)] = row[key];
        })
        return b;
      });
    } else {
      info(rows.affectedRows,"rows affected");
      return rows
    }
  } catch (err) {
    err.stack = err.stack + '\n    sql: ' + sql;
...
proto.queryOne = async function (sql, values) {
...
proto.selectPaginate = async function (table, options) {
  options = options || {};

  const totalCount = await this.count(table, options.where);

  const limit = options.pager.pageSize;
  const offset = (options.pager.currentPage - 1) * options.pager.pageSize;
  const sql = this._selectColumns(table, options.columns) +
    this._where(options.where) +
    this._orders(options.orders) +
...
proto.insert = async function (table, rows, options) {
  options = options || {};
  let firstObj;
  // insert(table, rows)
  ...
  if (!options.columns) {
    options.columns = Object.keys(firstObj).map(key => changeCase.snakeCase(key));
  }
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (let j = 0; j < options.columns.length; j++) {
      values.push(row[changeCase.camelCase(options.columns[j])]);
    }
    strs.push('(?)');
    params.push(values);
  }
...
proto.update = async function (table, row, options) {
  ...
  for (let i = 0; i < options.columns.length; i++) {
    ...
    sets.push('?? = ?');
    values.push(changeCase.snakeCase(column));
 ...
 */
proto.updateRows = async function (table, options) {
...
proto._where = function (_where) {
  if (!_where) {
    return '';
  }
  const where = {};
  let enableNull = _where.enableNull;
  Object.keys(_where).forEach(k => {
    if (k == 'enableNull') {
      return;
    }
    if (enableNull || !isEmpty(_where[k]))
      where[k] = _where[k];
  })
  ...
  for (const key in where) {
    const k = changeCase.snakeCase(key);
...
proto._orders = function (orders) {
  ...
  for (let i = 0; i < orders.length; i++) {
    ...
    if (typeof value === 'string') {
      values.push(this.escapeId(changeCase.snakeCase(value)));
      ...
      if (sort) {
        values.push(this.escapeId(changeCase.snakeCase(value[0])) + ' ' + sort);
      } else {
        values.push(this.escapeId(changeCase.snakeCase(value[0])));
      }
    }
  }
